...
void MDFNI_SaveMovie(char *fname, uint32 *fb, MDFN_Rect *LineWidths)
{
 ...
 memset(&RewindBuffer, 0, sizeof(StateMem));  // zero the memory pointed to by rewind buffer
...
fwrite(moviedata, 1, moviedatasize, inputbuffertest);
...
//PDCdata points to PortDataCache, getting the commands from this function out to the emulator
...
//this must be changed so that it reads from moviedata instead
