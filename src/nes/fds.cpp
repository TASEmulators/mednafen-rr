...
int FDS_DiskInsert(int oride)
{
	if(InDisk==255)
        {
         MDFN_DispMessage((UTF8 *)_("Disk %1$d Side %2$s Inserted"),SelectDisk>>1,(SelectDisk&1)?"B":"A");  
         InDisk=SelectDisk;
        }
        else   
        {
         MDFN_DispMessage((UTF8 *)_("Disk %1$d Side %2$s Ejected"),SelectDisk>>1,(SelectDisk&1)?"B":"A");
         InDisk=255;
        }
	return(TRUE);
}
...
int FDS_DiskSelect(void)
{
	if(InDisk!=255)
        {
         MDFN_DispMessage((UTF8 *)_("Eject disk before selecting."));
	 return(FALSE);
        }
        SelectDisk=((SelectDisk+1)%TotalSides)&3;
        MDFN_DispMessage((UTF8 *)_("Disk %1$d Side %2$s Selected"),SelectDisk>>1,(SelectDisk&1)?"B":"A");
	return(TRUE);
}
